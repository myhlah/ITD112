...
const DengueDataList = () => {
  ...
  //  bar chart
const barChartData = {
  labels: [...new Set(filteredData.map(data => data.location))],
  datasets: [
    {
      label: 'Cases',
      data: [...new Set(filteredData.map(data => data.location))]
        .map(location => filteredData.filter(data => data.location === location)
...
const barChartOptions = {
  ...
  // line chart
  const lineChartData = {
    labels: [...new Set(filteredData.map(data => data.date))],
    datasets: [
      {
        label: 'Cases',
        data: [...new Set(filteredData.map(data => data.date))]
          .map(date => filteredData.filter(data => data.date === date)
          .reduce((total, data) => total + data.cases, 0)),
        borderColor: 'rgba(75, 192, 192, 1)',
        backgroundColor: 'rgba(75, 197, 192, 1)',
        fill: false,
...
const scatterPlotOptions = {
  ...
  return (
    <div className="container">
      ...
      {/* Scatter plot */}
      <div className="chart-container">
      <h2 className="centered-title">Dengue Cases  Scatter Plot</h2>
